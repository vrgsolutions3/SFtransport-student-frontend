...
import { getFieldErrors, resendCodeSchema } from "@/lib/validation/auth";
...
export async function POST(request: NextRequest) {
  ...
  if (!checkRateLimit(`resend-code:${clientIp}`, 5, 60000)) {
  ...
  try {
    ...
    const bodyResult = resendCodeSchema.safeParse(rawBody);

    if (!bodyResult.success) {
      const fieldErrors = getFieldErrors(bodyResult.error);
      const firstError = Object.values(fieldErrors)[0] ?? "Dados invalidos para reenvio.";
